 ...
 * 文件作用: 压裂水平井页岩型复合模型核心计算类实现
 * 修改记录:
 * 1. [公式更新] 页岩型介质函数采用瞬态平板模型: f(s) = omega + sqrt(lambda(1-w)/3s) * tanh(...)
 */
...
QString ModelSolver19_36::getModelName(ModelType type, bool verbose)
{
    ...
    QString baseName = QString("页岩型储层试井解释%1").arg(modelId - 18);
    ...
    QString strMedium;
    if (type >= Model_19 && type <= Model_24) strMedium = "页岩型+页岩型";
    else if (type >= Model_25 && type <= Model_30) strMedium = "页岩型+均质";
    else strMedium = "页岩型+双重孔隙";
...
// [新增] 页岩型介质函数 - 瞬态平板模型 (Transient Slab Matrix)
// 公式: f(s) = w + sqrt( [lambda*(1-w)] / (3s) ) * tanh( sqrt( [3s(1-w)] / lambda ) )
    ...
    for (int i = 0; i < total_segments; ++i) {
        for (int j = i; j < total_segments; ++j) {
            ...
            if (i != j) {
                ...
                if (dist_centers > effectiveRadius + halfLen) {
                    A_mat(i, j) = 0.0;
                    A_mat(j, i) = 0.0;
                    continue;
                }
